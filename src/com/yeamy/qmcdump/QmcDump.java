...
public class QmcDump {
	...
	static {
		...
		for (int i = 0; i < ks.length; i++) {
			KEY[i] = (byte) (ks[i] & 0xFF);
		}
	}

	private static byte[] encrypt(int offset, byte[] buf, int len) {
		for (int i = 0; i < len; ++i) {
			buf[i] ^= mapL(offset + i);
		}
		return buf;
	...
	private static byte mapL(int v) {
		if (v >= 0) {
			if (v > 0x7FFF)
				v %= 0x7FFF;
		} else {
		...
		return (byte) (KEY[(v * v + 80923) & 0xFF]);
	}
	...
	public static boolean dump(File in, File outPath) {
		...
		case "qmc0":
		case "qmc3":
			return convert(in, new File(outPath, base + ".mp3"));
		case "qmcflac":
			return convert(in, new File(outPath, base + ".flac"));
		default:
			return false;
		}
	}
